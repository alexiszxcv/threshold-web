...
export default function MailroomPage() {
  ...
  const { loading, saveLetter } = useLetters()

  const handleSave = async () => {
    if (!letterContent.trim()) return

    try {
      await saveLetter(letterContent, recipient || undefined)
      setLetterContent("")
      setRecipient("")
    } catch (error) {
        ...
        <div className="mb-12">
          ...
          <div className="flex justify-center">
            <div className="w-4 h-16 bg-orange-300/40 rounded-full relative">
              <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 w-2 h-3 bg-orange-400/60 rounded-full animate-pulse"></div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-6">
          <Card className="bg-white/10 border-white/20 p-4">
            <input
              ...
              className="w-full bg-transparent border-none text-white placeholder-white/50 focus:outline-none text-lg"
